...
\begin{frame}{A simple \say{Hello World!}}
   ...
   \begin{tikzpicture}[@O]
      \node[below right,xshift=6mm,yshift=2.5cm,align=left,font=\itshape\footnotesize,gray] at(current page.west) {Compile by pressing the corresponding button\\or use \bbash{pdflatex}/\bbash{latexmk} from the commandline.};
   \end{tikzpicture}}
   \only<handout:2>{\eblLoadLtx{HelloWorld}{graphics options={trim=2cm 23cm 10cm 2cm, clip}}{}
   \begin{tikzpicture}[@O]
      \node[below right,xshift=6mm,yshift=-2cm,font=\itshape\footnotesize,gray] at(current page.west) {From now on, examples will be cropped automatically.};
   \end{tikzpicture}}
\end{frame}
...
\node[above,desc] (a) at(1.5,0.5) {The document class defines the basic layout of a document};
\draw[-Kite,w] (a) -- ++(.8,-1);
...
\draw [decorate,decoration={brace,amplitude=6pt,raise=4pt},yshift=0pt] (3.25,-1.05) -- ++(0,-1.5) node [desc,midway,right,xshift=0.45cm] {The main document};
\end{tikzpicture}
\end{center}
\end{frame}