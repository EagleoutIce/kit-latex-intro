...
\begin{itemize}
   ...
   \item In the long run, however, an installed version of \LaTeX{} has several benefits
   \begin{itemize}
      \itemsep4pt
...
\end{itemize}
\begin{tikzpicture}[@O]
   \node[above left=2mm,yshift=6.75mm] at(current page.south east) {\scalebox{.6}{\usebox\TempBoxA}};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{A simple \say{Hello World!}}
   \only<handout:1>{\vspace*{-2mm}\eblLoadLtx{HelloWorld}{}{righthand width=.3\linewidth}
   \begin{tikzpicture}[@O]
      \node[below right,xshift=4mm,yshift=2.5cm,align=left,font=\itshape] at(current page.west) {Compile by pressing the corresponding button\\or use \bbash{pdflatex}/\bbash{latexmk} from the commandline.};
   \end{tikzpicture}}
   \only<handout:2>{\eblLoadLtx{HelloWorld}{graphics options={trim=2cm 23cm 10cm 2cm, clip}}{}
   \begin{tikzpicture}[@O]
      \node[below right,xshift=3.5mm,yshift=-2cm] at(current page.west) {From now on, examples will be cropped automatically.};
   \end{tikzpicture}}
\end{frame}
